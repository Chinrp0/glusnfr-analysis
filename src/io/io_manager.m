...
%     % IO_MANAGER - Fixed file input/output operations
% 
...
%     io.getExcelFiles = @getExcelFiles;
...
% end
% 
% function [data, headers, success] = readExcelFile(filepath, useReadMatrix)
%     % FIXED: Excel reading with proper error handling
% 
%     success = false;
%     data = [];
%     headers = {};
...
% 
%     try
%         % Use readcell - most reliable for R2019a+
...
% 
%         % Convert to numeric with fixed logic
...
%     % FIXED: Numeric conversion without logical operator errors
% 
...
%                 % Try to convert string/char to number
%                 numValue = str2double(cellValue);
%                 if isfinite(numValue)
%                     numericData(row, col) = single(numValue);
%                 end
%             end
%             % For empty or non-convertible cells, leave as NaN
%         end
%     end
% end
% 
% function excelFiles = getExcelFiles(folder)
%     % Get Excel files with minimal validation
% 
%     if ~exist(folder, 'dir')
...
%     % FIXED: Validation without conflicting readcell parameters
% 
%     try
%         % Quick test read of first few cells (remove NumHeaderLines when using Range)
...
%     % FIXED: Extract valid headers using existing string_utils function
% 
...
%     end
% 
%     % Use existing string_utils to extract ROI numbers
%     try
%         cfg = GluSnFRConfig();
...
%             end
% 
%             fprintf('      Extracted %d valid ROI headers (ROI %d to %d)\n', ...
%                     length(validHeaders), min(roiNumbers), max(roiNumbers));
%         else
...
%             end
% 
%             fprintf('      Created %d sequential ROI headers (no ROI numbers found)\n', length(validHeaders));
%         end
% 
%     catch ME
...
%         end
% 
%         fprintf('      Created %d sequential ROI headers (string_utils failed: %s)\n', ...
%                 length(validHeaders), ME.message);
%     end
% end
% 
